...
const AVAILABLE_HOOKS = {
    ...
    'code-quality-primer': {
        ...
        description: 'Provides code quality context before edits'
    },
    'code-quality-validator': {
        event: 'PostToolUse',
        matcher: 'Write|Edit|MultiEdit',
        description: 'Enforces clean code standards on file edits'
    },
    'stop-validation': {
        event: 'Stop',
        description: 'Validates TypeScript and linting before allowing Claude to stop'
    },
    'pre-commit-check': {
        event: 'PreToolUse',
        matcher: 'Bash',
        pattern: '^git\\s+commit',
        description: 'Runs checks before git commits'
    },
    ...
    'pre-completion-quality-check': {
        ...
        description: 'Quality checks before marking todos complete or committing'
    },
    'post-write': {
        event: 'PostToolUse',
        matcher: 'Write',
        description: 'Runs actions after writing new files'
    },
    'pre-typescript-check': {
        event: 'PreToolUse',
        matcher: 'Bash',
        pattern: '^git\\s+commit',
        description: 'TypeScript validation before git commits'
    }
};
...
function addHook(settings, hookName) {
    ...
    let targetGroup = eventHooks.find(group => group.matcher === hookConfig.matcher &&
    ...
    if (!targetGroup) {
        ...
        if (hookConfig.matcher)
            targetGroup.matcher = hookConfig.matcher;
        if (hookConfig.pattern)
            targetGroup.pattern = hookConfig.pattern;
    ...
    while (true) {
        ...
        while (managingFile) {
            ...
            if (currentHooks.length > 0) {
                choices.push(new inquirer.Separator(chalk.yellow(`=== Current Hooks (${currentHooks.length}) ${chalk.gray('- press Enter to remove')} ===`)));
            ...
            switch (selection.action) {
                case 'remove':
                    ...
                    if (selectedPath)
                        saveSettings(selectedPath, settings);
                    break;
                case 'removeAll':
                    // Remove all hooks
                    settings.hooks = {};
