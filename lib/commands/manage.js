    ...
    while (true) {
        ...
        while (managingFile) {
            ...
            const saveHandler = async (selectedHookNames) => {
                ...
                const customHooksToPreserve = currentHooks.filter(hook => !(hook.name in AVAILABLE_HOOKS) && selectedHookNames.includes(hook.name));
